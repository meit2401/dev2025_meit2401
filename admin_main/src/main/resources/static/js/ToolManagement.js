...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (reprintModalEl && reprintOkButton && qrNumberInput && qrScanInput) {
        ...
        /**
         * 9桁の工具ラベルコード(16進数)を受け取り、再印刷Ajaxリクエストを実行する共通関数
         * @param {string} qrNumber - 9桁の16進数
        ...
        async function executeReprint(qrNumber) {
            ...
            if (qrNumber.length !== 9 || !/^[0-9A-Fa-f]{9}$/.test(qrNumber)) {
                alert('9桁の16進数(工具ラベルコード)を入力してください。');
                return false;
            }
        ...
        qrScanInput.addEventListener('input', function(e) {
            ...
            // 9桁の16進数かをチェック
            // 10桁対応は不要なため、T始まりなどのチェックは削除
            
            // 300ミリ秒後に入力がなければ、入力完了とみなして処理を開始
            reprintDebounceTimeout = setTimeout(async () => {
                try {
                    // 入力値が9桁の16進数パターンに一致する場合のみ実行
                    if (/^[0-9A-Fa-f]{9}$/.test(qrCodeData)) {
                        
                        const success = await executeReprint(qrCodeData);
                        ...
                        // 形式が合わない場合は無視するか、クリアしてフォーカスを維持
                        // (誤入力防止のため、明らかに桁数が多い場合などはクリアしても良い)
                        if (qrCodeData.length >= 9) {
                             qrScanInput.value = '';
                             qrScanInput.focus();
                        }
                    }
                } catch (err) {
                    console.error("スキャン解析エラー:", err);
                    qrScanInput.value = '';
                    qrScanInput.focus();
                }
